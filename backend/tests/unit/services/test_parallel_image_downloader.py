...
from services.image_downloader import download_images, download_single_image, MAX_CONCURRENT_DOWNLOADS, get_image_dir
...
class TestParallelImageDownloader:
    ...
    async def test_parallel_download_with_failures(self, httpx_mock):
        ...
        for metadata in successful_downloads:
            file_path = os.path.join(get_image_dir(), metadata["image_id"])
    ...
    async def test_download_single_image_retry_logic(self):
        ...
        with patch('httpx.AsyncClient') as mock_client_class:
            ...
            if result.get("image_id"):
                file_path = os.path.join(get_image_dir(), result["image_id"])
                if os.path.exists(file_path):
                    os.remove(file_path)
